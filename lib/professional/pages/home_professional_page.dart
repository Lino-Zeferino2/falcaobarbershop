...
class _HomeProfessionalPageState extends State<HomeProfessionalPage> {
  ...
  Widget build(BuildContext context) {
    print('HomeProfessionalPage: Building...');
                ...
                return ListTile(
                  leading: Stack(
                    children: [
                      Icon(
                        _pageIcons[index],
                        color: _selectedIndex == index ? Theme.of(context).colorScheme.primary : Theme.of(context).colorScheme.onSurface.withOpacity(0.7),
                      ),
                      if (index == 3 && _unreadNotificationsCount > 0) // Notificações index
                        Positioned(
                          right: 0,
                          top: 0,
                          child: Container(
                        ...
                        builder: (context) {
                          ...
                          if (_professionalData?.turnos != null) {
                            _professionalData!.turnos.forEach((key, value) {
                              String inicio = value['inicio'] ?? '';
                              String fim = value['fim'] ?? '';
                              String turnoName = key == 'manha' ? 'Manhã' : key == 'tarde' ? 'Tarde' : key;
                ...
                child: Row(
                  children: [
                    Icon(
                      ...
                      color: notification.lida ? Theme.of(context).colorScheme.onSurface.withOpacity(0.7) : Theme.of(context).colorScheme.primary,
                    ),
                    const SizedBox(width: 12),
                    Expanded(
                      child: Column(
  ...
  Widget _buildEditableProfileField(String label, String value, Function(String) onChanged) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Cliente: ${appointment.clientName}', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Row(
              children: [
  ...
  */
  ...
  void _showEditServiceDialog(AppointmentModel appointment) async {
    try {
      ...
      if (services.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Nenhum serviço disponível para alteração')),
        );
        return;
      }
      ...
      if (pickedTime != null) {
        ...
        final confirmed = await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            title: const Text('Confirmar Alteração'),
            content: Text(
              'Alterar horário para ${DateFormat('dd/MM/yyyy HH:mm').format(newDateTime)}?',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(context, false),
